...
const addEventListener1 = () => {
  ...
  if (isNotDuplicate) {
    cityTitle.insertAdjacentHTML(
      'beforeend',
      `<button type="button" id="${cityTag.value}" class="m-1 btn btn-primary">${cityTag.value}</button>`
    )
    document
      .querySelector(`#${cityTag.value}`)
      .addEventListener('click', (e) => {
        if (confirm('삭제하시겠습니까?')) {
          console.log(e, e.path, e.target)
          cityMember.splice(cityMember.indexOf(e.target.innerHTML), 1)
          e.target.remove()
        }
      })
...
function getServerData() {
  async function load() {
    try {
      ...
      alert(
        '데이터가 존재하지 않거나 잘못되었습니다.  가능한 날짜는 20년 3월 4일 ~ 21년 6월 9일입니다.'
      )
    }
  }
  load()
...
const checkIndex = (datas, cityIndex, totalContent) => {
  datas[0].body__items__item__gubun.data.forEach((data) => {
    cityIndex.map((index) => {
      if (data === datas[0].body__items__item__gubun.data[index])
        //해당 지역을 가진 인덱스라면
...
const pushValue = (totalContent, datas, data, index) => {
  for (let i = 0; i < datas.length; i++) {
    ...
    //배열형태이므로 나머지는 push메소드를 이용해 넣는다.
    else totalContent[data].push(datas[i].body__items__item__incDec.data[index])
  }
}
...
const setDate = () => {
  ...
  for (let i = 0; i < returnDay; i++) {
    ...
    Month =
      stringMonth.length == 1
        ? '0' + stringMonth.toString()
        : stringMonth.toString() // 한자리 수 인경우 앞에 0을 붙인다.
    Day =
      tDate.getDate().toString().length == 1
        ? '0' + tDate.getDate().toString()
        : tDate.getDate().toString()
